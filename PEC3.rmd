...
<div style="text-align: justify">
  ...
  Las llanuras de Rehoboth, en Namibia central, han permitido a algunos investigadores desarrollar estos nuevos modelos. Esta región semi-árida de estepa y poca vegetación ha sido durante cientos de años, y sigue siendo hoy en día, zona de pastoreo. A pesar de su contribución reducida al Producto Interior Bruto (PIB), la agricultura es un sector que se mantiene como fundamental para la mayoría de la población (algo más de dos millones de habitantes en 2019). De forma directa o indirecta, mantiene aproximadamente al 70% de la población del país. La elaboración de modelos que permitan realizar una explotación óptima de la estepa, a la vez que mantienen la dinámica de los pastizales, puede suponer enormes beneficios sobre la economía y salud de las personas. 
  

  Con el objetivo de estudiar los efectos del pastoreo y sus consecuencias sobre la composición de especies, la estructura vegetal, los patrones de diversidad o los parámetros fisicoquímicos, el enfoque de las "piosferas" ha sido empleado con frecuencia en regiones de clima semiárido en diferentes regiones del globo(Perkins & Thomas 1993; Landsberg et al. 2003; Todd 2006; Sasaki et al. 2008). Este documento trabaja con los datos obtenidos en un estudio realizado en la región de Rehoboth, el cual investiga el impacto ejercido por el pastoreo ganadero en la distribución de las características de algunas especies en el ecosistema semiárido de la savana africana (Welsus et al. 2012). El estudio emplea un conjunto jerárquico de modelos ecológicamente significativos con una complejidad creciente: el enfoque HOF (Huisman – Olff – Fresco) [enlace], para investigar las respuestas de pastoreo de las especies, los parámetros de diversidad y los valores de cobertura agrupados para dos rasgos: forma de crecimiento y ciclo de vida.  
<div/>  


References  

- Landsberg, J., James, C.D., Morton, S.R., Müller, W.J. & Stol, J. 2003. Abundance and composition of plant species along grazing gradients in Australian rangelands. Journal of Applied Ecology 40: 1008–1024.

- Perkins, J.S. & Thomas, D.S.G. 1993. Environmental responses and sensitivity to permanent cattle ranching, semi-arid western central Botswana. In: Thomas, D.S.G. & Allison, R.J. (eds.) Landscape sensitivity, pp. 273–286. Wiley, Chichester, UK.
...
- Welsus, et al. 2012. Disentangling plant trait responses to livestock grazing from spatio‐temporal variation: the partial RLQ approach. Journal of Vegetation Science 23: 98-113.



![](https://github.com/practicaUOC/trabajo_practica/blob/master/portada.jpg?raw=true)
...
```{r echo=F}
tab.1_cap<-table_nums(name=paste0("tab",ind.tabla))
sapply(piosphere$traits,class)%>% head()%>%  kable(caption = paste0(tab.1_cap,"Clasificación de las variables que definen los rasgos de las especies"),col.names = "Tipo de Variable") %>% kable_styling() %>%  add_footnote("Se muestran sólo seis primeras filas", notation="alphabet")
ind.tabla <- increment(ind.tabla,1)
```
...
tab.1_cap<-table_nums(name=paste0("tab",ind.tabla))
sapply(piosphere$env,class)%>% head() %>% kable(caption = paste0(tab.1_cap,"Clasificación de las variables que definen las variables medioambientales recogidas"),col.names = "Tipo de Variable") %>% kable_styling() %>%  add_footnote("Se muestran sólo seis primeras filas", notation="alphabet")
ind.tabla <- increment(ind.tabla,1)
```
...
- Mostrar el porcentaje de especies que tienen datos en cada una de las piosferas, y extraer el número de especies que la ocupan.

```{r echo=F}
tab.1_cap<-table_nums(name=paste0("tab",ind.tabla))
apply(piosphere$veg ,MARGIN = 1,FUN=function(x){round(100*(sum(x!=0)/ncol(piosphere$veg)),2)}) %>% head(10) %>%  kable(colnames="Porcentaje",caption = paste0(tab.1_cap,"Porcentaje de Especies con datos en cada piosfera")) %>% kable_styling()%>%  add_footnote("Se muestran sólo diez primeras filas", notation="alphabet")
ind.tabla <- increment(ind.tabla,1)
```
...
data.frame(`Especie Mayoritaria`=especie,Porcentaje=porcentaje) %>% head(10) %>%  kable(colnames=c("Especie Mayoritaria", "Porcentaje"),caption = paste0(tab.1_cap,"Especie mayoritaria en cada piosfera")) %>% kable_styling()%>%  add_footnote("Se muestran sólo diez primeras filas", notation="alphabet")
ind.tabla <- increment(ind.tabla,1)
```
...
data.frame(`Caracteristica`=caracteristicas.numericas,Especie=especie) %>% head(10) %>%   kable(colnames=c("Especie Mayoritaria", "Porcentaje"),caption = paste0(tab.1_cap,"Especies con mayores caracteristicas")) %>% kable_styling()%>%  add_footnote("Se muestran sólo diez primeras filas", notation="alphabet")
ind.tabla <- increment(ind.tabla,1)
```
...
```{r echo=F}
tab.1_cap<-table_nums(name=paste0("tab",ind.tabla))
piosphere$traits %>% dplyr::mutate(especie=rownames(.)) %>%  dplyr::filter(life.cycle=="annual") %>% dplyr::filter(leaf.consistency=="soft-mesomorphic") %>% dplyr::select(especie) %>% head(10) %>%   kable(colnames=c("Especie"),caption = paste0(tab.1_cap,"Especies ciclo vital anual y con consistencia de hoja mesomórfica suave")) %>% kable_styling() %>%  add_footnote("Se muestran sólo diez primeras filas", notation="alphabet")
ind.tabla <- increment(ind.tabla,1)
```

- Mostrar que piosferas tienen mayor inclinación de la media

```{r echo=F}
tab.1_cap<-table_nums(name=paste0("tab",ind.tabla))
media.inclinacion<-mean(piosphere$env$Inclination,na.rm=T)
piosphere$env %>% dplyr::mutate(piosfera=rownames(.)) %>%  dplyr::filter(Inclination>media.inclinacion) %>% dplyr::select(piosfera,Inclination) %>% head(10) %>%  kable(colnames=c("Piosfera","Inclination"),caption = paste0(tab.1_cap,"Piosferas con mayor inclinación que la media")) %>% kable_styling() %>%  add_footnote("Se muestran sólo diez primeras filas", notation="alphabet")
ind.tabla <- increment(ind.tabla,1)
```
...
- En relación al ejercicio anterior, correlacionar la presencia de nutrientes con la distribución del pH.

Como se verá más adelante en la práctica y ya adelantamos, el pH presenta una distribución asimétrica, con dos picos bien definidos. Puede resultar interesante comprobar si alguno de los nutrientes presenta asociación a dicha distribución. Escogemos para visualizar concretamente PO4, debido a su carácter limitante como factor de crecimiento. Nos interesa una primera aproximación visual a la posible correlación entre ambas variables.

```{r, echo =F,message=FALSE}
tab.1_cap<-table_nums(name=paste0("tab",ind.tabla))

#Gráficas para pH y PO4/pH
pHhist <- ggplot(data=env, aes(x=pH)) + 
  geom_histogram(col="black", 
                 fill="steelblue", 
                 alpha = .6) + 
  geom_vline(xintercept = median(env$pH), col = 2, lty = 2)+
  labs(x="pH", y="Frecuencia")
pHPO4 <- ggplot(data=env, aes(x=pH, y=PO4)) + 
...
#Texto pie de figura
text <- paste("Como podemos observar, parece haber una relación, ",
              "aunque ligera, entre ambas variables.", 
              "El pH presenta dos picos bien diferenciados y el PO4",
              "parece a su vez seguir cierta relación",
              "aunque sería necesaria una mayor confianza estadística",
              "para poder afirmarlo",
              sep = " ")
text.p <- ggparagraph(text = text, face = "italic", size = 11, color = "black")  

#Ensamblado
grid.arrange(pHhist, pHPO4, text.p,
             ncol=2, nrow=2,
             layout_matrix=rbind(c(1,2),
                                 c(3,3)),
             top = paste0(tab.1_cap,"Comparación PO4/pH"))
ind.tabla <- increment(ind.tabla,1)
```
...
Comprobamos la normalidad de las tres variables cuantitativas

```{r,echo=F}
test <- apply(traits[,c("height","SLA","diaspore.length")],MARGIN = 2,FUN = shapiro.test)
pander(test)
```



El p-valor es inferior al nivel de significación estándar 0.05 para las tres variables por lo que rechazamos H0 en todos los casos. Tenemos suficiente evidencia estadística para afirmar que estas variables no se ajustan a una distribución normal. Elaboramos un descriptivo y una representación gráfica para las variables.
...
plot.height<-ggplot(data=traits, aes(x=height)) + 
  geom_histogram(breaks=seq(0, 400, by=50), 
                 ...
                 alpha = .6,
                 binwidth=25) + 
  labs(title="Histograma Height", x="Altura", y="Frecuencia")
...
plot.sla<-ggplot(data=traits, aes(x=SLA)) + 
  geom_histogram(col="black", 
                 ...
                 alpha = .6,
                 binwidth=2) + 
  labs(title="Histograma SLA", x="SLA", y="Frecuencia")
...
logDHist <- ggplot(data=traits, aes(x=log(traits$diaspore.length))) + 
  geom_histogram(col="black", 
                 ...
                 alpha = .6,
                 binwidth=0.5) + 
  labs(title="Histograma Diaspore Length", x="Longitud de la diáspora", y="Frecuencia")
...
logDboxplot <- ggplot(data=traits, aes(y=log(traits$diaspore.length))) + 
  ...
  labs(title="Boxplot Diaspore length", y="Longitud de la diáspora")+
  coord_flip()

grid.arrange(plot.height, plot.sla, logDHist, logDboxplot,
             ncol=2, nrow=2,
             top = paste0(tab.1_cap,"Gráficos para height, SLA y Diaspore Length"))
...
Describimos las variables categóricas **Life Cycle**  y **Growth Form**  

Las tablas 17 y 18 presentan sus frecuencias, y la tabla 19 histogramas para ambas variables.  
...
table(traits$life.cycle) %>% 
  kable(col.names = c("Life Cycle","Freq"), caption = paste0(tab.1_cap,"Frecuencias absolutas Variable Life Cycle")) %>%
  kable_styling()
ind.tabla<-increment(ind.tabla,1)
tab.1_cap<-table_nums(name=paste0("tab",ind.tabla))
# table(traits$life.cycle)/sum(table(traits$life.cycle)) %>% kable() %>% kable_styling()
round(100*prop.table(table(traits$life.cycle)),4) %>% kable(col.names = c("Life Cycle","Proporción"), caption = paste0(tab.1_cap,"Frecuencias relativas  Life Cycle")) %>% kable_styling()
ind.tabla <- increment(ind.tabla,1)
```
...
```{r, echo=F}
tab.1_cap<-table_nums(name=paste0("tab",ind.tabla))
table(traits$growth.form) %>% kable(col.names = c("Growth Form","Freq"), caption = paste0(tab.1_cap,"Variable Growth Form")) %>% kable_styling()
# table(traits$life.cycle)/sum(table(traits$life.cycle)) %>% kable() %>% kable_styling()
...
tab.1_cap<-table_nums(name=paste0("tab",ind.tabla))
p2<-round(100*prop.table(table(traits$growth.form)),4) %>% data.frame() %>% ggplot(aes(x=Var1, y=Freq)) +
  geom_bar(stat="identity", fill="steelblue", alpha=.8)+
  theme_minimal()+xlab("Growth form")+ylab("Proporción")

grid.arrange(p, p2,
             ncol=2, nrow=1,
             top = paste0(tab.1_cap,"Histogramas para Lyfe Cycle y Growth Form"))
...
```{r, echo=F}
pander(apply(env[,c("pH","NO2","NO3","PO4")],MARGIN = 2,FUN = shapiro.test))
```
...
```{r cuatro_env, echo=F}
#Histograma
n1 <- ggplot(data=env, aes(x=pH)) + 
  geom_histogram(col="black", 
                 fill="steelblue", 
                 alpha = .6,
                 binwidth=.1) + 
  labs(title="Histograma pH", x="pH", y="Frecuencia")

n2 <- ggplot(data=env, aes(x=NO2)) + 
  geom_histogram(col="black", 
                 fill="darkorange", 
                 alpha = .6,
                 binwidth=.01) + 
...
grid.arrange(n1, n2, n3, n4, ncol=2, nrow=2,
             top=paste0(tab.1_cap,"Niveles de pH, NO2, NO3 Y PO4"))
```
...
Calcular número de especies por piosfera

```{r echo=F, message=F}
tab.1_cap<-table_nums(name=paste0("tab",ind.tabla))
num.especies<-apply(piosphere$veg,MARGIN = 1,FUN=function(x){sum(x!=0)})
df.modelo.lineal<-cbind(num.especies,piosphere$env)
shapiro.test(df.modelo.lineal$num.especies) %>% pander()
```


```{r echo=F, fig.cap="Histograma del Número de Especies"}

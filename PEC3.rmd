...
```{r echo=F}
tab.1_cap<-table_nums(name=paste0("tab",ind.tabla))
apply(piosphere$veg ,MARGIN = 1,FUN=function(x){round(100*(sum(x!=0)/ncol(piosphere$veg)),2)}) %>% kable(colnames="Porcentaje",caption = paste0(tab.1_cap,"Porcentaje de Especies con datos en cada piosfera")) %>% kable_styling()
ind.tabla <- increment(ind.tabla,1)
```


- Podemos asumir que las variaciones internas para las diferentes subáreas son significativas?
...
data.frame(`Especie Mayoritaria`=especie,Porcentaje=porcentaje) %>% kable(colnames=c("Especie Mayoritaria", "Porcentaje"),caption = paste0(tab.1_cap,"Especie mayoritaria en cada piosfera")) %>% kable_styling()
ind.tabla <- increment(ind.tabla,1)
```
...
data.frame(`Caracteristica`=caracteristicas.numericas,Especie=especie) %>% kable(colnames=c("Especie Mayoritaria", "Porcentaje"),caption = paste0(tab.1_cap,"Especies con mayores caracteristicas")) %>% kable_styling()
ind.tabla <- increment(ind.tabla,1)
```
...
```{r echo=F}
tab.1_cap<-table_nums(name=paste0("tab",ind.tabla))
piosphere$traits %>% dplyr::mutate(especie=rownames(.)) %>%  dplyr::filter(life.cycle=="annual") %>% dplyr::filter(leaf.consistency=="soft-mesomorphic") %>% dplyr::select(especie) %>% kable(colnames=c("Especie"),caption = paste0(tab.1_cap,"Especies ciclo vital anual y con consistencia de hoja mesomórfica suave")) %>% kable_styling()
ind.tabla <- increment(ind.tabla,1)
```
...
piosphere$env %>% dplyr::mutate(piosfera=rownames(.)) %>%  dplyr::filter(Inclination>media.inclinacion) %>% dplyr::select(piosfera,Inclination) %>% kable(colnames=c("Piosfera","Inclination"),caption = paste0(tab.1_cap,"Piosferas con mayor inclinación que la media")) %>% kable_styling()
ind.tabla <- increment(ind.tabla,1)
```

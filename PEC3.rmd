...
- Landsberg, J., James, C.D., Morton, S.R., Mu¨ ller, W.J. & Stol, J. 2003. Abundance and composition of plant species along grazing gradients in Australian rangelands. Journal of Applied Ecology 40: 1008–1024.
...
```{r}
tab.1_cap<-table_nums(name=paste0("tab",ind.tabla))
apply(piosphere$veg ,MARGIN = 1,FUN=function(x){round(100*(sum(x!=0)/ncol(piosphere$veg)),2)}) %>% head(10) %>%  kable(colnames="Porcentaje",caption = paste0(tab.1_cap,"Porcentaje de Especies con datos en cada piosfera")) %>% kable_styling()
ind.tabla <- increment(ind.tabla,1)
```
...
data.frame(`Especie Mayoritaria`=especie,Porcentaje=porcentaje) %>% head(10) %>%  kable(colnames=c("Especie Mayoritaria", "Porcentaje"),caption = paste0(tab.1_cap,"Especie mayoritaria en cada piosfera")) %>% kable_styling()
ind.tabla <- increment(ind.tabla,1)
```
...
data.frame(`Caracteristica`=caracteristicas.numericas,Especie=especie) %>% head(10) %>%   kable(colnames=c("Especie Mayoritaria", "Porcentaje"),caption = paste0(tab.1_cap,"Especies con mayores caracteristicas")) %>% kable_styling()
ind.tabla <- increment(ind.tabla,1)
```
...
```{r echo=F}
tab.1_cap<-table_nums(name=paste0("tab",ind.tabla))
piosphere$traits %>% dplyr::mutate(especie=rownames(.)) %>%  dplyr::filter(life.cycle=="annual") %>% dplyr::filter(leaf.consistency=="soft-mesomorphic") %>% dplyr::select(especie) %>% head(10) %>%   kable(colnames=c("Especie"),caption = paste0(tab.1_cap,"Especies ciclo vital anual y con consistencia de hoja mesomórfica suave")) %>% kable_styling()
ind.tabla <- increment(ind.tabla,1)
```
...
piosphere$env %>% dplyr::mutate(piosfera=rownames(.)) %>%  dplyr::filter(Inclination>media.inclinacion) %>% dplyr::select(piosfera,Inclination) %>% head(10) %>%  kable(colnames=c("Piosfera","Inclination"),caption = paste0(tab.1_cap,"Piosferas con mayor inclinación que la media")) %>% kable_styling()
ind.tabla <- increment(ind.tabla,1)
```
...
table(traits$life.cycle) %>% 
  kable(col.names = c("Life Cycle","Freq"), caption = paste0(tab.1_cap,"Variable Life Cycle")) %>%
  kable_styling()
...
```{r}
df %>% dplyr::group_by(habitat) %>% summarize(media=round(mean(GrazInt,na.rm=T),4),mediana=median(GrazInt,na.rm=T),sd=round(sd(GrazInt,na.rm=T),4)) %>% kableExtra::kable(caption="Media, Mediana y Desviación Típica por habitat") %>% kableExtra::kable_styling()
```
...
Para utilizar la primera estrategía voy a probar
...
Analizamos la normalidad de la variable transformada
...
```{r}
df.sin.atipicos %>% dplyr::group_by(habitat) %>% summarize(media=round(mean(GrazInt,na.rm=T),4),mediana=median(GrazInt,na.rm=T),sd=round(sd(GrazInt,na.rm=T),4)) %>% kableExtra::kable(caption="Media, Mediana y Desviación Típica por habitat") %>% kableExtra::kable_styling()
```
...
Aplicar test no paramétricos:


```{r}
kruskal.test(df$GrazInt~df$habitat)
compareGroups::compareGroups(formula=habitat~GrazInt,method = 4,data=df) %>% compareGroups::createTable(show.p.mul = T,digits=5) %>% compareGroups::export2md()
```

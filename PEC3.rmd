...
Como se verá más adelante en la práctica y ya adelantamos, el pH presenta una distribución asimétrica, con dos picos bien definidos. Puede resultar interesante comprobar si alguno de los nutrientes presenta asociación a dicha distribución. Escogemos para visualizar concretamente PO4, debido a su carácter como factor limitante del crecimiento. Nos interesa una primera aproximación visual a la posible correlación entre ambas variables.
...
```{r}
tab.1_cap<-table_nums(name=paste0("tab",ind.tabla))
traits %>% dplyr::select(height,SLA,diaspore.length) %>% psych::describe() %>% kableExtra::kable(caption = "Descripción de variable height,SLA y Diaspore Length") %>% kableExtra::kable_styling()
ind.tabla <- increment(ind.tabla,1)
```
...
logDboxplot <- ggplot(data=traits, aes(y=log(traits$diaspore.length))) + 
  ...
  labs(title="Longitud de las diásporas", y="Longitud")


cowplot::plot_grid(plot.height, plot.sla, logDHist, logDboxplot)
...
Describimos las variables categóricas **Lyfe Cycle**  y **Growth Form** 
...
```{r}
tab.1_cap<-table_nums(name=paste0("tab",ind.tabla))
env %>% dplyr::select(pH,NO2, NO3, PO4) %>% psych::describe() %>% kableExtra::kable(caption = "Descripción de variable pH,NO2,NO3,PO4") %>% kableExtra::kable_styling()
ind.tabla <- increment(ind.tabla,1)
```
